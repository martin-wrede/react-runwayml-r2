...
// .wrangler/tmp/bundle-TQN8Gt/checked-fetch.js
...
// .wrangler/tmp/pages-rogb2I/functionsWorker-0.06224217282644062.mjs
...
async function onRequest3({ request, env }) {
  ...
  try {
    ...
    const submitResponse = await fetch("https://api.runwayml.com/v1/generations", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${env.RUNWAYML_API_KEY}`,
        "Content-Type": "application/json"
      ...
      body: JSON.stringify({
        ...
        // Make sure this model is allowed in your plan
        input: {
          prompt,
          seed: 42,
          motion: "default",
    ...
    if (!submitData.id) {
      return new Response(JSON.stringify({ error: "Failed to start generation", details: submitData }), {
        status: 500,
    ...
    console.error("\u26A0\uFE0F RunwayML video generation error:", error);
    return new Response(
      JSON.stringify({ error: error.message }),
      {
        status: 500,
...
async function onRequest4({ request, env }) {
  ...
  try {
    ...
    v;
    const submitResponse = await fetch("https://api.runwayml.com/v2/generations", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${env.RUNWAYML_API_KEY}`,
        "Content-Type": "application/json"
    ...
    while (attempts < 20) {
      await new Promise((r) => setTimeout(r, 3e3));
      const statusResponse = await fetch(`https://api.runwayml.com/v2/generations/${generationId}`, {
        headers: {
          "Authorization": `Bearer ${env.RUNWAYML_API_KEY}`
        }
      });
    ...
    console.error("\u26A0\uFE0F RunwayML error:", error);
...
async function onRequest5({ request, env }) {
  ...
  try {
    ...
    if (action === "generate") {
      ...
      if (!result.ok) {
        throw new Error(`RunwayML API error: ${result.status}`);
      }
...
// .wrangler/tmp/bundle-TQN8Gt/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-TQN8Gt/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.06224217282644062.js.map
